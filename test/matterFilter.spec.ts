import { isValidMatterArticle } from "../backend/sources/matter/filter";
...
describe("filter", () => {
	it("filters out articles with read as true", () => {
		...
		for (let i = 0; i < 20; i++) {
			...
			articleList.push(
				generateMatterTestArticle({ read, inQueue: true, wordCount: 1000 }),
			);
		}
	...
	it("filters out articles with inQueue as false", () => {
		...
		for (let i = 0; i < 20; i++) {
			...
			articleList.push(
				generateMatterTestArticle({ inQueue, read: false, wordCount: 1000 }),
			);
		}
	...
	it("filters out articles with less than 500 words", () => {
		...
		for (let i = 0; i < 20; i++) {
			...
			articleList.push(
				generateMatterTestArticle({ inQueue: true, read: false, wordCount }),
			);
		}
